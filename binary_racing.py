...
def extract_yellow(image):
    ...
    return yellow_image
def crop_with_margin(image, margin=15, output_path='cropped_with_margin.png'):
...
def extract_number_from_screenshot(image):
    custom_config = r'--psm 8 -c tessedit_char_whitelist=0123456789 --tessdata-dir "D:\Program Files\Tesseract-OCR\tessdata"'
    text = pytesseract.image_to_string(image, config=custom_config, lang='digits')
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if result:
            if len(result) > 1:
                print("Found more then 1 number?")
                print(result)
                exit()
            binary = format(result[0], '08b')
